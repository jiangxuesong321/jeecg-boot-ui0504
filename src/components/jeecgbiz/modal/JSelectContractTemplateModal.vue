...
<!--    cancelText="关闭"-->
<!--  >-->
  <a-drawer title="选择合同模板" :width="width" placement="right" :closable="false" :headerStyle="{ textAlign: 'center'}"
            @close="close" destroyOnClose :visible="visible">
    <a-card :bordered="false">
      <a-form-model>
        <a-row>
          <a-col :span='6'>
            <a-form-model-item label="模板名称" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-model='queryParam.temName'></a-input>
            </a-form-model-item>
          </a-col>
          <a-col :span='6'>
            <a-form-model-item label="模板编码" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-model='queryParam.temNo'></a-input>
            </a-form-model-item>
          </a-col>
          <a-col :span='6'>
...
export default {
    name: 'JSelectContractTemplateModal',
    components: {},
    ...
    methods: {
      initUserNames() {
        if (this.userIds) {
          ...
          getAction('/srm/basContractTemplate/getMultiContract', param).then((list)=>{
            this.selectionRows = []
            let selectedRowKeys = []
            let textArray = []
      ...
      async loadData(arg) {
        ...
        this.loading = true
        params.status = '1';
        getAction('/srm/basContractTemplate/list', params).then(res=>{
          if (res.success) {
