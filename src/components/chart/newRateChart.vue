<template>
	<div style="width: 200px; height: 200px;">
		<div class="percentloop">
			...
			<div class="number">
				<!-- <div>{{ title }}</div> -->
				<div style="width: 100%;text-align: center;font-weight: bold;color: #3662EC;font-size: 25px;">{{ percent }} %</div>
			</div>
		</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			rotateLeft(deg) { // 大于180时，执行的动画
			  if(this.$refs.leftcontent != null && this.$refs.leftcontent != undefined){
          this.$refs.leftcontent.style.transform = 'rotate(' + (deg - 180) + 'deg)'
        }

			},
			rotateRight(deg) { // 小于180时，执行的动画
			  if(this.$refs.rightcontent != null && this.$refs.rightcontent != undefined){
