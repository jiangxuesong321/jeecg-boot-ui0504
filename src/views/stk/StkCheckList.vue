    ...
    <div>
      ...
      <a-table
        ...
        <template slot="projectName" slot-scope="text,record">
          <div style="width: 120px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" :title="text">
            {{text}}
          </div>
        </template>
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <a-divider type="vertical" v-if="record.checkStatus =='0'  && roleCode.indexOf(record.checkApprove) > -1 && record.checkApprove == 'project_center'"/>
          <a @click="handleApprove(record)" v-if="record.checkStatus =='0'  && roleCode.indexOf(record.checkApprove)  > -1 && record.checkApprove == 'project_center'">确认</a>

<!--          <a-divider type="vertical" v-if="record.checkStatus =='0' && record.checkApprove == 'prod_line' && roleCode.indexOf(record.checkApprove) > -1" />-->
<!--          <a @click="handleApprove(record)" v-if="record.checkStatus =='0' && record.checkApprove == 'prod_line' && roleCode.indexOf(record.checkApprove) > -1">确认</a>-->

<!--          <a-divider type="vertical" v-has="'io:cost'"/>-->
<!--          <a @click="handleCost(record)" v-has="'io:cost'">费用录入</a>-->
  ...
  let columns = [
    ...
    // },
    {
      ...
      customRender:function (text,record) {
        if(text == '0'){
          let t = '待验收';
          if(record.checkApprove == 'prod_line'){
            t = t + '(产线确认)';
          }else if(record.checkApprove == 'project_center'){
            t = t + '(项目中心确认)';
          }
          return t;
        }else{
