  ...
  <a-spin :spinning="confirmLoading">
    <a-tabs v-model="activeKey" @change="handleChangeTabs">
        ...
        <j-form-container>
          ...
          <a-form-model ref="form" :model="model" :rules="validatorRules" slot="detail">
              ...
              <a-row>
                ...
                <a-col :span="8">
                  <a-form-model-item label="招标类型" :labelCol="spans.labelCol3" :wrapperCol="spans.wrapperCol3">
                    ...
                    <j-dict-select-tag type="select" v-model="model.biddingType" dictCode="bidding_type"
                      placeholder="请输入招标类型" :disabled="formDisabled" />
                  </a-form-model-item>
                </a-col>
                <a-col :span="8">
              <a-form-model-item label="项目分类" :labelCol="spans.labelCol3" :wrapperCol="spans.wrapperCol3" required>
                <a-tree-select
                ...
                <a-col :span="8">
                  <a-form-model-item label="开标方式" :labelCol="spans.labelCol3" :wrapperCol="spans.wrapperCol3"
                    ...
                    <j-dict-select-tag type="select" v-model="model.openBiddingType" dictCode="open_bidding_type"
                      placeholder="请输入开标方式" :disabled="formDisabled" />
                  </a-form-model-item>
                </a-col>
                ...
                <a-col :span="8">
                  <a-form-model-item label="开标地址" :labelCol="spans.labelCol3" :wrapperCol="spans.wrapperCol3"
                    ...
                    <j-dict-select-tag v-model="model.openBiddingAddress" placeholder="请输入开标地址"
                      dictCode="sys_depart,depart_name,id,parent_id = '-1'" :disabled="formDisabled" />
                  </a-form-model-item>
                </a-col>
                <a-col :span="8">
                  <a-form-model-item label="投标截至时间" :labelCol="spans.labelCol3" :wrapperCol="spans.wrapperCol3"
                    prop="biddingDeadline">
                    <j-date placeholder="请选择投标截至时间" v-model="model.biddingDeadline" style="width: 100%"
                ...
                <a-col :span="8">
                  <a-form-model-item label="开标时间" :labelCol="spans.labelCol3" :wrapperCol="spans.wrapperCol3"
                    ...
                    <j-date placeholder="请选择开标时间" v-model="model.openBiddingDate" style="width: 100%"
                      :disabled="formDisabled" />
                  </a-form-model-item>
                </a-col>
                <a-col :span="8">
                  <a-form-model-item label="评标时间" :labelCol="spans.labelCol3" :wrapperCol="spans.wrapperCol3" prop="date">
                    <a-range-picker v-model="model.date" format="YYYY-MM-DD" :placeholder="['开始时间', '结束时间']"
                      @change="onBirthdayChange" :disabled="formDisabled" />
                  </a-form-model-item>
                </a-col>
                <a-col :span="8">
                  <a-form-model-item label="是否显示供应商排名" :labelCol="spans.labelCol3" :wrapperCol="spans.wrapperCol3"
                    prop="isSort">
                ...
                <a-col :span="8">
                  <a-form-model-item label="是否多次报价" :labelCol="spans.labelCol3" :wrapperCol="spans.wrapperCol3"
                    prop="isQuotes">
            ...
            <a-card class="card" title="招标明细" :bordered="false" style="margin-top: 15px;">
              <a-table v-if="model.reqType == '0'" ref="table" size="middle" bordered rowKey="id"
                class="j-table-force-nowrap" :scroll="{ x: true }" :columns="ptype == 'normal' ? columns : columns5"
                :dataSource="dataSource" :row-selection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange }"
                :pagination="false">
                <template slot="qty" slot-scope="text,record">
                  <a-input-number v-model="record.qty" :disabled="formDisabled" :max="record.maxQty - record.purcQty"
                    v-if="ptype != 'change'"></a-input-number>
                  <span v-else>
                    {{ text }}
                  </span>
                </template>
              ...
              <a-table v-if="model.reqType != '0'" ref="table" size="middle" bordered rowKey="id"
                class="j-table-force-nowrap" :scroll="{ x: true }"
                :row-selection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange }"
                :columns="ptype == 'normal' ? columns1 : columns6" :dataSource="dataSource" :pagination="false">
                <template slot="qty" slot-scope="text,record">
                  <a-input-number v-model="record.qty" :disabled="formDisabled" :max="record.maxQty - record.purcQty"
                    v-if="ptype != 'change'"></a-input-number>
                  <span v-else>
            ...
            <a-card class="card" title="邀请供应商" :bordered="false" style="margin-top: 15px;">
              <a-button type="primary" style="float: right;overflow: hidden;margin-bottom: 10px;z-index: 99999"
                @click="selectSupp" :disabled="formDisabled">选择供应商</a-button>
              <a-table ref="table" size="middle" bordered rowKey="code" class="j-table-force-nowrap" :scroll="{ x: true }"
                :columns="columns2" :dataSource="suppList" :pagination="false">
                <template slot="action" slot-scope="text,record,index">
                  <a @click="delRow(index)" :disabled="formDisabled">删除</a>
                </template>
              </a-table>
              <j-select-supplier-modal ref="suppForm" modalWidth="80%" :multi="true"
                @ok="toBack"></j-select-supplier-modal>
            </a-card>
            <!-- 子表单区域 -->
              ...
              <a-row>
                <a-col :span="24">
                  <a-form-item label="投标须知" :labelCol="spans.labelCol1" :wrapperCol="spans.wrapperCol1">
                    <a-input v-model="model.comment" placeholder="请输入投标须知" :rows="3" type='textarea'
                      :disabled="formDisabled">
                    </a-input>
                  </a-form-item>
                </a-col>
      ...
      <a-tab-pane tab="评标模板" key="1">
        ...
        <j-modal width="30%" :visible="visible" title="评标类型" switchFullscreen wrapClassName="j-user-select-modal"
          @cancel="close" style="top:50px" cancelText="关闭">
        ...
        <div v-if="checkValues != null && checkValues.indexOf('0') > -1">
          <a-row>
            ...
            <a-col :span="6">
              <span>
                权重
                <a-input-number style="width: 40%" v-model="weight[checkValues.indexOf('0')]" :min="0" :max="100"
                  disabled />
              </span>
            </a-col>
            <a-col :span="6"></a-col>
            <a-col :span="6">
              <a-button type="primary" style="float: right" @click="openTemp('0')"
                :disabled="formDisabled">选择评分模板</a-button>
            </a-col>
          </a-row>

          <a-table style="margin-top: 10px" ref="table" size="middle" :scroll="{ x: true }" bordered :columns="columns3"
            :dataSource="templateList" :pagination="false" class="j-table-force-nowrap">
            ...
            <template slot="itemName" slot-scope="text,record,index">
              ...
              <a-input v-model="record.itemName" :maxLength="50" :disabled="formDisabled"
                style="width: 80%;margin-left: 10px"></a-input>
            </template>
            ...
            <template slot="itemScore" slot-scope="text,record,index">
              <a-input-number v-model="record.itemScore" style="width: 100%" :disabled="formDisabled"
                @change="changeVal('0')" :min="0" :max="100"></a-input-number>
            </template>

            <template slot="professionalId" slot-scope="text,record,index">
              <span
                v-if="record.professionalName != null && record.professionalName != '' && record.professionalName != undefined">
                <span v-for="(item, index) in record.professionalName.split(',')">
                  <a-tag color="#2db7f5">{{ item }}<a-icon type="close" @click="delChild(item, '0', index)"
                      v-if="!formDisabled" /></a-tag>
                </span>
              </span>
            </template>
          </a-table>
        ...
        <div v-if="checkValues != null && checkValues.indexOf('1') > -1" style="margin-top: 40px">
          <a-row>
            ...
            <a-col :span="6">
              <span>
                权重
                <a-input-number style="width: 40%" v-model="weight[checkValues.indexOf('1')]" :min="0" :max="100"
                  disabled />
              </span>
            </a-col>
            <a-col :span="6"></a-col>
            <a-col :span="6">
              <a-button type="primary" style="float: right" @click="openTemp('1')"
                :disabled="formDisabled">选择评分模板</a-button>
            </a-col>
          </a-row>
          <a-table style="margin-top: 10px" ref="table" size="middle" :scroll="{ x: true }" bordered :columns="columns4"
            :dataSource="templateList1" :pagination="false" class="j-table-force-nowrap">
            <template slot="action" slot-scope="text,record,index">
            ...
            <template slot="itemScore" slot-scope="text,record,index">
              <a-input-number v-model="record.itemScore" style="width: 100%" :disabled="formDisabled"
                @change="changeVal('1')" :min="0" :max="100"></a-input-number>
            </template>
            ...
            <template slot="professionalId" slot-scope="text,record,index">
              <span
                v-if="record.professionalName != null && record.professionalName != '' && record.professionalName != undefined">
                <span v-for="(item, index) in record.professionalName.split(',')">
                  <a-tag color="#2db7f5">{{ item }}<a-icon type="close" @click="delChild(item, '1', index)"
                      v-if="!formDisabled" /></a-tag>
                </span>
              </span>
            </template>
          ...
          <div v-if="!formDisabled" @click="addRow('1')">
            <a-icon type="plus-circle" />
            <span style="font-size:16px;matgin-top:20px;color: #00348b;position: relative;"
              :disabled="formDisabled">添加</span>
          </div>
        </div>
