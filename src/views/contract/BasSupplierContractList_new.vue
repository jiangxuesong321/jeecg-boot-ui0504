      ...
      <div>
        ...
        <a-table
          ...
          <template slot="prodName" slot-scope="text,record">
            <div v-if="text != null && text != undefined && text != ''" v-for="(item,index) in text.split(',')">
              <div style="width: 160px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" :title="item">
                {{item}}
              </div>
            </div>
          </template>
          ...
          <template slot="contractPriceTax" slot-scope="text,record">
            <div v-if="text != null && text != undefined && text != ''" v-for="(item,index) in text.split(',')" style="text-align: right">
          ...
          <template slot="qtyNum" slot-scope="text,record">
            <div v-if="text != null && text != undefined && text != ''" v-for="(item,index) in text.split(',')">
              {{iegAmount(Math.trunc(item),'total')}}
            </div>
          </template>
...
<script>
  ...
  let columns = [
    ...
    },
    {
      ...
      width:120,
      scopedSlots: { customRender: 'contractPriceTax' }
    },
    {
      title:'设备数量',
      align:"center",
      ...
      dataIndex: 'qtyNum',
      width:120,
      scopedSlots: { customRender: 'qtyNum' }
    },
    {
      title:'品牌',
      align:"center",
      ...
      dataIndex: 'prodBrand',
      width:160,
      scopedSlots: { customRender: 'prodBrand' }
    },
    {
      title:'供应商名称',
      align:"center",
  ...
  export default {
    ...
    methods: {
      open1(expanded, row) {
        row.childList = []
        if (expanded) {
          let url = "/srm/contractBase/fetchChildQtyList";
          let param = {
            id: row.id
          }
          getAction(url, param).then(res => {
            row.childList = res.result
            this.$forceUpdate();
          })

          this.expandedRowKeys1 = []
          this.expandedRowKeys1.push(row.id)
        } else {
          this.expandedRowKeys1 = []
        }
      },
      open(expanded, row) {
        row.childList = []
        if (expanded) {
          let url = "/srm/contractBase/fetchChildList";
          let param = {
            id: row.id
          }
          getAction(url, param).then(res => {
